...
class Driver:
    ...
    def __init__(self, root, userInput):
        ...
        self._name = "File Handling Programming Exercises"
        pass
    ...
    def run(self):
        if self.parms["run"] == "False":
            ...
            results = fh_02.exercise_14_read_first_n_lines(self.root)
            results = fh_02.exercise_15_read_last_n_lines(self.root)
            results = fh_02.exercise_16_read_line_numbers_from_file(self.root)
            results = fh_02.exercise_17_find_longest_word(self.root)
            results = fh_02.exercise_18_letter_frequency(self.root)
            ...
            results = fh_03.exercise_24_copy_file_using_binary_mode(self.root)
            results = fh_03.exercise_25_rename_single_file(self.root)
            results = fh_03.exercise_26_rename_multiple_files(self.root)
            results = fh_03.exercise_27_delete_a_file(self.root)
            results = fh_03.exercise_28_merge_multiple_files(self.root)
            results = fh_03.exercise_29_reverse_line_order(self.root)
            results = fh_03.exercise_30_list_all_files(self.root)


            logger.info("")
            logger.info("#####################################################")
            logger.info(f"{self._name} - 31 through 34")
            logger.info("#####################################################")
